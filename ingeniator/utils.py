...
def toy_feature_selection_dataset(
    as_frame=True,
    ...
    X = np.concatenate(
        [signal, np.random.rand(num_samples, num_features - signal_features)], axis=1
    )
    ...
    if not as_frame:
        ...
        columns = [
            f"signal_{i+1}" if i < signal_features else f"noise_{i-signal_features+1}"
